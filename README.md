...
- **Slack `/casestudy` slash command + modal as a fully-internal product surface.** Workflow 20 wraps the W14 case-study brief sub-workflow in a Slack-interactivity frontend so anyone in the marketing channel can type `/casestudy`, fill a 9-field modal (workflow JSON + project name + 9-option industry select + three optional metric fields + two optional context textareas), and walk away — 5-7 minutes later a publication-ready 800-1200 word case study lands in Notion `AI Case Study Library` with `Status = Needs Review` and a Slack DM back to the submitter. Two webhooks (slash command + modal-submit), one `views.open` API call, two `Respond to Webhook` acks placed *before* the heavy work to honor Slack's 3-second SLA — same primitives W15 (the Slack Modal Router) provides as a reusable pattern. The pipeline runs in two layers: a brief pass (Claude Sonnet 4.5 orchestrator + the W14 sub-workflow tool with 6 parallel specialists) that produces structured metadata, then a long-form pass (Claude Sonnet 4.5 writer → **Gemini 3 Flash hallucination-detector critic** → Claude Sonnet 4.5 editor → 7.5/10 quality gate with loop-back). The defining engineering choice is the **`dataConfidence` partition**: a JS workflow analyzer (deterministic, non-LLM) splits every fact into `verified` (from the JSON), `userProvided` (from the modal), and `needsPlaceholder` (everything else), and every downstream prompt is given that bucket structure verbatim. The critic's job is then mechanical — any specific dollar amount or percentage not in `userProvided` is a hallucination, full stop — which is what stops the autonomous draft from inventing ROI numbers no one can defend. Output writes to Notion as `databasePage` create + a raw `PATCH /v1/blocks/{id}/children` chunked into ≤2000-char paragraph blocks (Notion's per-block cap). *See [`docs/workflows/transform-labs-case-study-generator.md`](docs/workflows/transform-labs-case-study-generator.md#the-dataconfidence-partition).*
