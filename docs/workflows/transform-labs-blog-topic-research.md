...
The workflow as exported has a few **historical drifts** worth fixing on the first import:

- **Sticky-note dates are stale.** The `Sticky Note - Trigger` says *"Every Friday at 3:00 PM"* but the actual schedule trigger is Mondays at 12:00 UTC. The `Sticky Note - Output` says the destination is *"Google Sheets → Prompts tab"* but the actual destination node is the Notion `AI Blog - Prompts` database. The `Sticky Note - AI Generation` says *"Azure OpenAI (gpt-5-mini)"* but the actual chat model is Google Gemini 3 Pro. These are documentation drift, not code bugs — the sticky notes were written for an earlier version of the workflow and never re-synced. Fix the stickies on import or remove them; the executable nodes are the source of truth.
- **Markdown auto-link artifacts in code blocks.** A handful of identifiers in the JS nodes and the Slack template have been corrupted to markdown-link form by a paste through a markdown-aware editor — `feeds.map` reads as `[feeds.map](http://feeds.map)`, `articles.map` reads as `[articles.map](http://articles.map)`, `json.data` reads as `[json.data](http://json.data)`, etc. As written these will throw at runtime. Search the imported JSON for `](http://` and remove the wrapping `[...](http://...)` syntax, leaving the bare identifier. Affected nodes: `Define RSS Feeds`, `Filter & Dedupe Articles`, `Send a message`.
