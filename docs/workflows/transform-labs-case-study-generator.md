...
- **Slack-as-frontend pattern with the 3-second ack contract.** Two webhooks (slash command + modal submission), one `views.open` API call, two `Respond to Webhook` acks placed before any heavy work. Same primitives as W15 (the Slack Modal Router) but used here as a *consumer* — the modal's `callback_id = casestudy_form` is exactly the callback the W15 router watches for. Operator types `/casestudy`, fills a form, gets a "Processing... 5-7 minutes" DM and walks away; nothing more to do.
- **Two-layer agent architecture: brief (specialists + orchestrator) → long-form (writer + critic + editor + quality gate).** The brief pass produces structured metadata; the long-form pass turns it into prose with hallucination + voice + format gating. Brief generation calls W14 as a `toolWorkflow`, so this workflow is the *user-facing wrapper* over the case-study brief sub-workflow. Reuse, not reimplementation.
- **`dataConfidence` as a first-class object that flows through every prompt.** The JS workflow analyzer partitions every fact into `verified` / `userProvided` / `needsPlaceholder`. The brief writer, the case-study writer, the critic, and the editor are all given the same buckets. The critic's job is mechanical: any specific metric not in `userProvided` is a hallucination. This is what stops the autonomous draft from inventing ROI numbers.
- **Hallucination-detector critic prompt.** Gemini 3 Flash on the critic (cheaper, faster, mechanical task), with a system prompt that opens *"YOU ARE A HALLUCINATION DETECTOR"* and rules that flag invented dollar amounts and percentages as `CRITICAL`. Cross-vendor judging — Anthropic writer + Google critic + Anthropic editor — same lesson as W6 / W12, applied to the honesty axis instead of the style axis.
- **Bounded writer-critic-editor loop with a 7.5/10 quality gate.** `Update Case Study` outputs the revised draft; an `IF` node on `overallScore ≥ 7.5` either advances to publish or loops back to the critic for another pass. Worst-case cost is bounded; the brief in `$('Generate Case Study BRIEF3')` is durable, so a bad long-form pass doesn't wipe the structured upstream output.
- **First-person voice enforcement.** All four agent system prompts encode *"YOU ARE TRANSFORM LABS, writing about YOUR OWN work. We built — not Transform Labs built."* The critic checks this as a hard category; the editor's revision priorities list *"FIX VOICE — Must be first-person"* second only to fixing hallucinations.
- **Notion writeback as two API calls (databasePage create + blocks/children PATCH).** The n8n Notion node creates the title row with `Status = Needs Review`; a JS `Prepare Blocks` chunks the prose into ≤2000-char paragraph blocks; a raw HTTP `PATCH /v1/blocks/{pageId}/children` appends the body. Splitting it keeps the metadata write atomic if the body append fails.
- **Workflow Analyzer as a deterministic non-LLM pass.** Before any LLM call, a JS code node walks the pasted workflow JSON, categorizes every node into `triggers / aiNodes / integrations / outputs / dataProcessing / utilities`, computes a complexity score from a fixed formula (`nodes×0.25 + ai×2 + branches×0.75 + integrations×0.3 + errorHandling + dataProcessing×0.3`), classifies the workflow into one or more named patterns (`Feed Monitoring / API Handler / Scheduled Job / Data Pipeline / Notification System / AI-Powered / Multi-Destination`), and reads sticky-note documentation if present. Deterministic facts feed the LLMs; the LLMs aren't asked to count nodes or describe topology.
- **`[ESTIMATED: X — TO VERIFY]` placeholder markers as a first-class output element.** The editor is forbidden from filling them in. Whoever opens the case study sees them at a glance and knows what to verify before publishing — same honesty layer as W14.
