...
tokens. Parameterized for the four variables that change weekly: volume,
topic, date, byline. See SKILL.md for full spec.
...
FLAG_GOLD  = (255, 199, 44)            # Pantone 1235
...
def add_aurora(sky):
    layers = [
        (11, H * 0.22, 80, 65, 105, AURORA_BANDS[0], 50),
        (23, H * 0.18, 50, 95, 95,  AURORA_BANDS[1], 60),
        (37, H * 0.30, 100, 50, 75, AURORA_BANDS[2], 35),
    ]
...
def render(volume, topic, date, byline, kicker, out_path,
    ...
    font_topic = fraunces(96, weight=900, opsz=144, font_paths=fonts)
    line_h = 110
...
def main():
    ...
    p.add_argument("--out",    default=None,
                   help="Output PNG path. Default derived from --volume.")
    args = p.parse_args()

    # Real-string newlines for the topic (CLI passes literal "\n")
