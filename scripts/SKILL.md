...
- `--out` Output PNG path. Default `./alaska-ai-brief-vol-{N}.png` derived from volume number.
...
- Topic headline in Fraunces Black 96pt, white, multi-line if needed
- Italic gold motto beneath headline, default `what's moving in alaska ai, this week`
- Footer band at y=H-110 with byline left, coordinates right
- Coordinates stamp `61°13′N · 149°54′W` (Anchorage, do not change)
...
The build script auto-downloads fonts to `./fonts/` on first run if missing.

## How to invoke

```bash
# Default content (test render):
